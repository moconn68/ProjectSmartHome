...
5. Run the following command: `airodump-ng -c <channel> --bssid <BSSID> -w /root/Desktop/ <interface-mon>`. This will intercept network handshakes necessary for cracking the password and store them on the desktop. Be sure to use the correct channel number for the access point for the -c argument. 
